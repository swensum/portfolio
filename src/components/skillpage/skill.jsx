      ...
      <section className={`skills-section ${isSkillVisible ? 'fade-in' : ''}`}>
        <div className="skill-row">
          <div className="skills-container">
            <p className={`eyebrow ${isSkillVisible ? 'fade-in' : ''}`}>// Skills</p>
            ...
            <h2 className={`skills-heading ${isSkillVisible ? 'fade-in' : ''}`}>
              <span className="heading-text">
                ...
                <span className="heading-line">
                  areas<span className="heading-underline"></span>
                </span>
              </span>
            </h2>

            <p className={`expert ${isSkillVisible ? 'fade-in' : ''}`}>
              I work across the mobile and web stack, from Flutter and React interfaces to
              Firebase-backed data, and I care as much about clean, maintainable code as I do
              about how the finished product feels to use.
            </p>

            <button
              type="button"
              className={`certificate ${isSkillVisible ? 'fade-in' : ''}`}
              onClick={() => setShowModal(true)}
            >
              Certificates
              <svg width="22" height="21" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="cert-icon">
  ...
  style={{
    '--tab-index':
      hoveredTab !== null
        ...
        : TABS.findIndex((tab) => tab.id === activeButton),
  }}
...
>
  ...
  {TABS.map((tab, index) => {
    const highlightedIndex =
      hoveredTab !== null
        ? TABS.findIndex((item) => item.id === hoveredTab)
        : TABS.findIndex((item) => item.id === activeButton);
    ...
    return (
      ...
      >
        {tab.label}
            ...
            <div className="tab-content" key={activeButton}>
              {renderContent()}
            </div>
          </div>
        </div>
